function [net,stats] = MWCNN_CSNet_train_dag(net, varargin)
...
for epoch=start+1:opts.numEpochs
    ...
    if  mod(epoch,10)~=1 && isfield(imdb,'set') ~= 0
        ...
        clear imdb;
        %[imdb] = generatepatches;
        [imdb]  = load('E:\Research_DLCS\TrainingPatches\imdb_256_64_stride24.mat'); 
    end
...
try
    input   = gpuArray(imdb.labels(:,:,:,batch));
catch
    input   = gpuArray(imdb.inputs(:,:,:,batch));
end
rng('shuffle');
mode    = randperm(8);
input   = data_augmentation(input, mode(1));
label   = input; 
inputs2 = {'input', input, 'label', label} ;
